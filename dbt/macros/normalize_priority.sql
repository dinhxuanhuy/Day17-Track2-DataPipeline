...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or trim({{ col }}) = '' then 'priority bị null hoặc rỗng'
        when try_cast({{ col }} as integer) is not null then 'priority là số nhưng ngoài khoảng 1..4'
        else 'priority là nhãn chuỗi không xác định'
    end
{% endmacro %}
